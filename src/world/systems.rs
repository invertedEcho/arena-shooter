...
use crate::{
    ...
    world::{
        ...
        components::DebugPoint,
        messages::{SpawnDebugPointMessage, SpawnMapMessage},
...
pub fn spawn_map(
    ...
    mut spawn_map_message_reader: MessageReader<SpawnMapMessage>,
    selected_map_state: Res<State<SelectedMapState>>,
) {
    for _ in spawn_map_message_reader.read() {
        let selected_map_state = selected_map_state.get().clone();
        commands.spawn((
        ...
        if selected_map_state == SelectedMapState::MediumPlastic {
            ...
            for (name, maybe_constructor) in collider_rules {
                match maybe_constructor {
                    ...
                    None => {
                        collider_hierarchy = collider_hierarchy
                            .without_constructor_for_name(name);
                    }
                }
            }
